...
- `ceo` now provides the first executive operating layer above governed lab runs. It runs bounded blocks, writes company-status, decision-packet, heartbeat-status, and stop-request artifacts, separates research-infra/understanding/chart-facing deltas, flags fake progress, and keeps product candidates in champion/challenger shadow mode until a promotion proposal is explicitly approved. Overnight CEO mode should be driven by Codex thread heartbeats that inspect the latest packet and choose one next bounded block, not by a blind multi-hour Python loop.
