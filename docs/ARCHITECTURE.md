...
- `ceo_ops.py`: executive layer above governed `lab_ops`. It runs bounded CEO blocks, aggregates company-level status, writes decision packets, heartbeat status, and stop requests, tracks research-infra/understanding/product deltas, flags fake progress, and keeps chart-facing candidates in champion/challenger shadow mode until a promotion proposal is approved.
