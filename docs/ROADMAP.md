...
- `ceo` now provides the first executive operating layer above governed lab runs. It runs bounded blocks, writes company-status, decision-packet, heartbeat-status, stop-request, binding-action, self-audit, capability-gap, and champion/challenger artifacts, separates research-infra/understanding/chart-facing deltas, flags fake progress, and keeps product candidates in champion/challenger shadow mode until a promotion proposal is explicitly approved. Overnight CEO mode should be driven by Codex thread heartbeats that inspect the latest packet and run one `ceo execute-next` action, not by a blind multi-hour Python loop or by manually falling back to `run-block`.
