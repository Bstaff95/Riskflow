...
- The new CEO eval suite is the first objective 9.9-readiness harness. It scores replayability, state-machine transition legality, action-contract consistency, dispatch-receipt backing/fingerprints, approval gating, production guardrails, fresh/withheld validation authority, role-result closure, trace-grade usability, evidence-debt visibility, mission coverage, and strategy-capital attention allocation from artifacts a fresh session can inspect. Hard failures can block dispatch; advisory strategy-capital gaps lower 9.9 readiness without becoming red blockers by themselves.
...
- Run-generated promotion proposals now require specialist review closure: validation referee plus product translator or risk officer.
